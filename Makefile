...
.PHONY: help helm-lint helm-template helm-template-broker helm-template-rabbitmq helm-test helm-dry-run helm-package \
        helm-template-full helm-template-mount-env helm-template-env-keys test-helm \
        helm-install helm-upgrade helm-uninstall helm-status \
        run-local validate-broker-yaml validate-adapter-yaml validate
...
helm-template-full: ## Render helm templates with full configuration
	...
	helm template $(RELEASE_NAME) $(CHART_DIR) \
		...
		--set broker.googlepubsub.createSubscriptionIfMissing=true \
		--set broker.subscriber.parallelism=20 \
		--set broker.logConfig=true \
		--set hyperfleetApi.baseUrl=https://api.hyperfleet.example.com \
		--set autoscaling.enabled=true \
		--set podDisruptionBudget.enabled=true \
		--set podDisruptionBudget.minAvailable=1

helm-template-mount-env: ## Render helm templates with broker.mountAsEnv=true (mount entire ConfigMap)
	@echo "$(GREEN)Rendering helm templates (mountAsEnv=true)...$(NC)"
	helm template $(RELEASE_NAME) $(CHART_DIR) \
		--set broker.create=true \
		--set broker.type=googlepubsub \
		--set broker.googlepubsub.projectId=test-project \
...
helm-template-env-keys: ## Render helm templates with broker.envKeys (specific keys as env vars)
	@echo "$(GREEN)Rendering helm templates (envKeys)...$(NC)"
	helm template $(RELEASE_NAME) $(CHART_DIR) \
		--set broker.create=true \
		--set broker.type=googlepubsub \
		--set broker.googlepubsub.projectId=test-project \
		...
		--set broker.googlepubsub.subscription=test-subscription \
		--set broker.mountAsEnv=true \
		--set 'broker.envKeys={BROKER_TYPE,BROKER_HOST}'

test-helm: helm-lint helm-template-broker helm-template-rabbitmq helm-template-mount-env helm-template-env-keys ## Run all helm chart tests (lint + template rendering)
...
validate-broker-yaml: ## Validate charts/configs/broker.yaml syntax
	@echo "$(GREEN)Validating charts/configs/broker.yaml...$(NC)"
	@cat charts/configs/broker.yaml | yq '.' > /dev/null && echo "broker.yaml is valid YAML" || echo "broker.yaml has syntax errors"
...
validate: validate-broker-yaml validate-adapter-yaml ## Validate all YAML files
